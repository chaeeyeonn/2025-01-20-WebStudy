...
public class MusicTypeFind extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<a href=MainServlet?mode=3&cno=1 class=\"btn btn-sm btn-danger\">가요<a>");
		out.println("<a href=MainServlet?mode=3&cno=2 class=\"btn btn-sm btn-success\">Top50<a>");
		...
		out.println("<a href=MainServlet?mode=3&cno=7 class=\"btn btn-sm btn-default\">Classic<a>");
		out.println("</div>");
		...
		if(startPage>1)
		{
		  out.println("<li><a href=\"MainServlet?mode=3&cno="+cno+"&page="+(startPage-1)+"\">&lt;</a></li>");// 이전 블록 / 블록 중 마지막 페이지로 이동(이전 startPage-1)
		}
		
		for(int i=startPage;i<=endPage;i++)
		{
		 if(i==curpage)
		  out.println("<li class=active><a href=\"MainServlet?mode=3&cno="+cno+"&page="+i+"\">"+i+"</a></li>");// active: 현재 페이지 버튼 색깔 바꾸기
		 else
		  out.println("<li><a href=\"MainServlet?mode=3&cno="+cno+"&page="+i+"\">"+i+"</a></li>");
		}
		if(endPage<totalpage)
		{
			out.println("<li><a href=\"MainServlet?mode=3&cno="+cno+"&page="+(endPage+1)+"\">&gt;</a></li>");// 다음 블록 / 블록 중 처음 페이지로 이동(이전 endPage+1)
		}
        out.println("</ul>");
