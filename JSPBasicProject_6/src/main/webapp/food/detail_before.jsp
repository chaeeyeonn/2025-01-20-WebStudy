...
<%-- 
        Cookie(교재301page~)
        HTTP 프로토콜은 새로운 페이지가 실행되면 기존의 입력된 모든 정보가 사라진다
        -----------로그인 => 새로운 페이지로 이동(로그인 정보는 사라진다)
                   지속성(유지성)이 없다
                   ----------------정보가 계속 유지될 수 있게 만든 라이브러리 
                                     ==> Session / Cookie
                                         (장바구니,예약정보,...)
             => 상태의 지속성
                ---데이터의 변경 상황
                React / Vue => state관리 프로그램(데이터 상태 프로그램)
             ...
             => 사용법
               1. 저장
                  new Cookie(키, 값)
                           ...
                           => Map 방식
                           => key 중복 불가능 (중복된 키는 저장 불가)
                  - response: 한 개 JSP에서 한 가지 일만 수행
                                        -----------
                                        HTML 전송 => 2
                                        Cookie 전송 => 1
                                        => 여러 번 전송 가능
               ...
               2. 읽기: Cookie[] cookies=request.getCookies()
                      @CookieValue => String
               3. 삭제: setMaxAge(0)
               4. 저장기간 지정: setMaxAge(초단위로 기입) => 60*60*24 => default : 30분
               5. 저장된 데이터 읽기
                 => 키 읽기: getName()
                 => 값 읽기: getValue()
               6. 저장 경로 설정: setPath("/")
