...
public class RecipeDAO {
   ...
   FROM (SELECT no,title,poster,chef,hit,likecount,replycount
   FROM recipe))
   WHERE num BETWEEN #{start} AND #{end}
  </select>
  <select id="recipeTotalPage" resultType="int">
   SELECT CEIL(COUNT(*)/12.0) FROM recip
	 */
   ...
   SELECT no,chef,poster,mem_cont1,mem_cont2,mem_cont3,mem_cont7,mem_cont2,num 
   FROM (SELECT no,chef,poster,mem_cont1,mem_cont2,mem_cont3,mem_cont7,mem_cont2,rownum as num
   FROM (SELECT no,chef,poster,mem_cont1,mem_cont2,mem_cont3,mem_cont7,mem_cont2
